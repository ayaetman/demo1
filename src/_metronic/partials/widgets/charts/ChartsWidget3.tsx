...
const ChartsWidget3: React.FC<Props> = ({className}) => {
  ...
  <div className='card-header border-0 pt-5'>
    <h3 className='card-title align-items-start flex-column'>
      <span className='card-label fw-bolder fs-3 mb-1'>{t('Recent Transactions')}</span>

      <span className='text-muted fw-bold fs-7'> {t('More than')} <span>1000</span> {t('new records')}</span>
    </h3>
...
function getChartOptions(height: number): ApexOptions {
  ...
  return {
    ...
    xaxis: {
      categories: [t('Feb'), t('Mar'), t('Apr'), t('May'), t('Jun'), t('Jul'), t('Aug')],
      axisBorder: {
        show: false,
      },
      axisTicks: {
